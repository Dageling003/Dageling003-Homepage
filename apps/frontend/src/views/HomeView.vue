...
const configHandlers: Record<string, (val: string) => void> = {
  ...
  professions: (v) => { try { professions.value = JSON.parse(v) as string[] } catch (e) { console.error('[HomeView] professions parse failed:', e) } },
  links: (v) => { try { links.value = (JSON.parse(v) as Array<{ text: string; color: string; url: string }>).filter((l) => !isPlaceholderUrl(l.url)) } catch (e) { console.error('[HomeView] links parse failed:', e) } },
  techs: (v) => { try { techs.value = JSON.parse(v) as Array<{ name: string }> } catch (e) { console.error('[HomeView] techs parse failed:', e) } },
  todos: (v) => { try { todos.value = JSON.parse(v) as Array<{ text: string; done: boolean }> } catch (e) { console.error('[HomeView] todos parse failed:', e) } },
  typewriterWords: (v) => { try { typewriterWords.value = JSON.parse(v) as string[] } catch (e) { console.error('[HomeView] typewriterWords parse failed:', e) } },
