...
describe('AuthService', () => {
  ...
  describe('validateUser', () => {
    it('should return user for valid credentials', async () => {
      ...
      const result = await service.validateUser({ username: 'admin', password: 'password123456' })
  ...
  describe('login', () => {
    it('should return accessToken and username', async () => {
      mockUsersRepo.findOne.mockResolvedValue(mockUser)
      const result = await service.login({ username: 'admin', password: 'password123456' })
  ...
  describe('resetPassword', () => {
    it('should throw for non-existent token', async () => {
      ...
      await expect(service.resetPassword('invalid-token', 'newpassword1234')).rejects.toThrow(
        BadRequestException,
      )
    })
    ...
    it('should throw for already used token', async () => {
      ...
      await expect(service.resetPassword('used-token', 'newpassword1234')).rejects.toThrow(
        BadRequestException,
      )
    })
    ...
    it('should throw for expired token', async () => {
      ...
      await expect(service.resetPassword('expired-token', 'newpassword1234')).rejects.toThrow(
        BadRequestException,
      )
    })
    ...
    it('should reset password for valid token', async () => {
      ...
      mockUsersRepo.save.mockResolvedValue(true)
      mockResetTokenRepo.save.mockResolvedValue(true)
      const result = await service.resetPassword('valid-token', 'newpassword1234')
      expect(result.message).toContain('成功')
    })
  })
