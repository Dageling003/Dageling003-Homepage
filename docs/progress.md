...
| v0.7.1 | 2026/06/08 | 默认入口调整为后台（`/` → 301 → `/admin/`），前台主页搬到 `/site/` |
...
- [x] Caddyfile：`/` 301 跳转到 `/admin/`，用户首次访问默认进入后台
- [x] 前台主页（`apps/frontend`）的 `vite.base` 从 `/` 改为 `/site/`
- [x] Caddyfile 增加 `handle_path /site*` 提供前台静态资源
- [x] `deploy.sh` 完成提示更新为新的访问地址
- [x] `README.md` / `docs/architecture.md` / `docs/dev-guide.md` 同步更新

访问路径：

| 入口 | 开发 | 生产 |
|------|------|------|
| 后台管理（默认） | http://localhost:3001 | `https://<域名>/` → `/admin/` |
| 前台主页 | http://localhost:3000 | `https://<域名>/site/` |
